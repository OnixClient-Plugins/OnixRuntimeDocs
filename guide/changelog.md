...
- The constructor of [OnixModuleSettingRedirector](xref:OnixRuntime.Api.OnixClient.OnixModuleSettingRedirector) taking an [OnixModule](xref:OnixRuntime.Api.OnixClient.OnixModule) now also takes a bool.
  ...
  - This is important because if you start wrapping Onix Client modules in a class like that, you don't want a deleted setting to be re-added automatically linking to nothing.
...
### New Features
- [Item](xref:OnixRuntime.Api.Items.Item) now has a [DescriptionIdentifier](xref:OnixRuntime.Api.Items.Item.DescriptionIdentifier) property. This is used in the game's jsons and lang files.
- [Block](xref:OnixRuntime.Api.World.Block) now has a [DescriptionIdentifier](xref:OnixRuntime.Api.World.Block.DescriptionIdentifier) property. This is used in the game's jsons and lang files.
...
- [RuntimeWorld](xref:OnixRuntime.Api.World.RuntimeWorld) now has a [IsHardcore](xref:OnixRuntime.Api.World.RuntimeWorld.IsHardcore) which you can get AND set :D. 
  - Setting it on the client will require trust and probably not do much other than change the client-side UI.
  - Setting it on the server won't immediately do anything but after a relog it should apply.
- All Onix Settings and Modules have a ToString method now.
...
- A few of [OnixModuleSettingRedirector](xref:OnixRuntime.Api.OnixClient.OnixModuleSettingRedirector) now have an AfterSetting bool you can set to have them take effect after the setting.
...
- [OnixModule](xref:OnixRuntime.Api.OnixClient.OnixModule) now has a [IsBlockedEnabled](xref:OnixRuntime.Api.OnixClient.OnixModule.IsBlockedEnabled) to reflect recent changes in the blocking system.
...
- The [OnixPluginInstaller](xref:OnixRuntime.Plugin.OnixPluginInstaller) now has a few more functions to help with the multiple sources.
...
- The Modules API now uses shared memory address containers for all modules, fixing a lot of issues with the previous runtime.
  - (1k changes moment...) 
  - This fixes AddSetting but also anything else that was broken that I was aware of.
